...
function LivePageClient() {
  ...
  const fetchLiveSources = async () => {
    try {
      ...
      if (isTauriEnv) {
        // Tauri 环境：直播功能需要云端配置，显示提示信息
        setLiveSources([]);
        setLoading(false);
        setError('直播功能需要云端配置，当前为本地模式');
        return;
      }
      ...
      if (runtimeStorageType === 'localstorage') {
        ...
        setLiveSources([]);
        setLoading(false);
        setError('请通过 Tauri 应用访问此功能');
        return;
      }

      // 云端模式：获取 AdminConfig 中的直播源信息
  ...
  useEffect(() => {
    const preload = async () => {
      ...
      const precheckUrl = `/api/live/precheck?url=${encodeURIComponent(
        videoUrl,
      )}&quantumtv-source=${currentSourceRef.current?.key || ''}`;
