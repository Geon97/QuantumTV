...
/**
 * 带超时的 fetch 请求
 * 在 Tauri 环境下使用 fetch_url 命令绕过 CORS
 */
...
function getDoubanProxyConfig(): {
  ...
  // Tauri 环境下不使用 direct 模式（需要服务端 API），改用 CDN
  const runtimeStorageType =
    (window as any).RUNTIME_CONFIG?.STORAGE_TYPE || 'localstorage';
  if (runtimeStorageType === 'localstorage' && doubanProxyType === 'direct') {
    return {
