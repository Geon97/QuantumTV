  ...
  const checkVersion = async () => {
    ...
    try {
      ...
      if (isTauriEnv || runtimeStorageType === 'localstorage') {
        ...
        const result: VersionCheckResult = await checkForUpdates();
        ...
        const remoteTimestamp = result.remoteTimestamp;

        // 构造响应数据格式
        const data: VersionCheckResponse = {
          success: true,
          current: {
            version: '1.0.0',
            timestamp: localTimestamp,
            displayVersion: `v1.0.0+${localTimestamp}`,
          ...
          remote: remoteTimestamp
            ? {
                ...
                downloadUrl: 'https://github.com/Geon97/QuantumTV',
              }
            : undefined,
        };
      ...
      {hasUpdate && remote && (
        <div className='p-4 bg-orange-50 dark:bg-orange-900/20 border-2 border-orange-200 dark:border-orange-800 rounded-lg'>
          <div className='flex items-start justify-between'>
            ...
            <a
              href={
                remote.downloadUrl || 'https://github.com/Geon97/QuantumTV'
              }
              target='_blank'
